...
def build_function(args, index, dockerfile, aws_config, application_name, function_name, detailed_dir):
    ...
    image_uri = '{}.dkr.ecr.{}.amazonaws.com/lambda-ecache-{}-{}'.format(aws_config['account_id'], aws_config['region'], args.graph_name, function_name)
    ...
    print('====== Upload to ECR ======')
    os.system('aws ecr batch-delete-image --repository-name lambda-ecache-{}-{} --image-ids imageTag={} --region {}'.format(args.graph_name, function_name, index, aws_config['region']))
    time.sleep(10)
    os.system('docker push {}:{}'.format(image_uri, index))
    time.sleep(10)
    print('====== Create Lambda Function ======')
    os.system('aws lambda delete-function --function-name ecache{}_{}_{} --region {}'.format(args.graph_name, function_name, index, aws_config['region']))
    time.sleep(10)
    os.system('aws lambda create-function --function-name ecache{}_{}_{} --role {} --package-type Image --code ImageUri={}:{} --timeout 900 --memory-size 3538 --vpc-config SubnetIds={},SecurityGroupIds={} --region {}'.format(
        args.graph_name,
        function_name,
        index,
        aws_config['role_arn'],
...
def main():
    ...
    for func_name in ['bfs']:
        os.system('aws ecr create-repository --repository-name lambda-ecache-{}-{}'.format(args.graph_name ,func_name))
        index = 0
        print(index)
        detailed_dir = ''
        if func_name == 'bfs':
