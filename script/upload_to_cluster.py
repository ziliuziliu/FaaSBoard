...
# Redis Cluster configuration for AWS ElastiCache
REDIS_CLUSTER_STARTUP_NODES = [
    ClusterNode("graph-0001-001.graph.hcdnu5.apse1.cache.amazonaws.com", 6379),
    ClusterNode("graph-0001-002.graph.hcdnu5.apse1.cache.amazonaws.com", 6379),
    ClusterNode("graph-0001-003.graph.hcdnu5.apse1.cache.amazonaws.com", 6379),
    ClusterNode("graph-0002-001.graph.hcdnu5.apse1.cache.amazonaws.com", 6379),
    ClusterNode("graph-0002-002.graph.hcdnu5.apse1.cache.amazonaws.com", 6379),
    ClusterNode("graph-0002-003.graph.hcdnu5.apse1.cache.amazonaws.com", 6379),
    ClusterNode("graph-0003-001.graph.hcdnu5.apse1.cache.amazonaws.com", 6379),
    ClusterNode("graph-0003-002.graph.hcdnu5.apse1.cache.amazonaws.com", 6379),
    ClusterNode("graph-0003-003.graph.hcdnu5.apse1.cache.amazonaws.com", 6379)
]
REDIS_PASSWORD = None
...
def connect_to_redis_cluster():
    try:
        client = RedisCluster(
            startup_nodes=REDIS_CLUSTER_STARTUP_NODES,
            ...
            reinitialize_steps=None  # Disable auto-reinit to simplify troubleshooting
        )
        
        # Verify cluster connection
        client.ping()
        logger.info("Successfully connected to AWS ElastiCache Redis Cluster with TLS")
...
def upload_folder_to_redis_cluster(redis_client, local_folder_path):
    try:
        ...
        logger.info(f"Successfully uploaded {file_count} files to AWS ElastiCache Redis Cluster")
        return True

    except Exception as e:
        logger.error(f"Error uploading data to Redis Cluster: {e}")
        return False


def main():
    if len(sys.argv) != 2:
        print("Usage: python upload_to_cluster.py <folder_name>")
        sys.exit(1)
    ...
    redis_client = connect_to_redis_cluster()
    if not redis_client:
        sys.exit(1)

    success = upload_folder_to_redis_cluster(redis_client, str(target_folder_path))
    if not success:
        sys.exit(1)
